...
def _compute_embedding_mp_worker(model_path: str, image_paths: List[str]) -> Dict[str, np.ndarray]:
...
def compute_embeddings_mp(model_path: str, image_paths: List[str], num_processes: int = 6) -> Dict[str, np.ndarray]:
    if not image_paths:
        return {}
    ...
    results = process_map(worker_func, chunks, max_workers=num_processes, desc="Computing embeddings", unit="chunk")
